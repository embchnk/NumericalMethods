...
/***************************************************************************/
void Zadanie1::readFromDat() {
	std::ifstream file;
	file.open("dane_5a.dat");
	double uTemp, vTemp, max, temp;
	bool maskTemp;
	max = 0.;
	int i, j;

	while(file >> i >> j >> uTemp >> vTemp >> maskTemp) {
		u[j][i] = uTemp;
		v[j][i] = vTemp;
		temp = sqrt(uTemp * uTemp + vTemp * vTemp);
		if (temp > max)
			max = temp;
		mask[j][i] = maskTemp;
	}
...
void Zadanie1::exc() {
	...
	file.open("dat/z3_integrals.dat");
	std::string s = "dat/z3_it";
	int counter = 0;
	...
	for (double n = 0; n < 100; n += dt) {
		...
		for (j = 1; j < ny; ++j) {
			ro2[j][0] = 0.25 * (ro[j][1] + ro[j][nx] + ro[j - 1][0] + ro[j + 1][0]) -
										dt * (u[j][0] * (ro[j][1] - ro[j][nx]) +
													v[j][0] * (ro[j + 1][0] - ro[j - 1][0])) / 2 * d; 
			ro2[j][nx] = 0.25 * (ro[j][0] + ro[j][nx - 1] + ro[j - 1][nx] + ro[j + 1][nx]) -
										dt * (u[j][nx] * (ro[j][0] - ro[j][nx - 1]) +
													v[j][nx] * (ro[j + 1][nx] - ro[j - 1][nx])) / 2 * d; 		}
		for (j = 1; j < ny; ++j) {
			for (i = 1; i < nx; ++i) {
				if (mask[j][i]) {
					ro2[j][i] = 0.25 * (ro[j][i + 1] + ro[j][i - 1] + ro[j - 1][i] + ro[j + 1][i]) -
										dt * (u[j][i] * (ro[j][i + 1] - ro[j][i - 1]) +
													v[j][i] * (ro[j + 1][i] - ro[j - 1][i])) / 2 * d; 
					roPrev = ro;
					ro = ro2;
				}
			}
